  ...
  actions: {
    ...
    async LoginAuthentication(context, payload) {
      try {
        ...
        if (data) {
          console.log(data);
          context.commit("setUser", data.session.access_token);
          if (isSave) {
          ...
          if (user) {
            try {
              const { data: profileData, error: profileError } = await supabase
                .from("users")
                .select("fristName, surName, avatar")
                .eq("id", userId)
                .single();
              if(profileError){
                console.log('fail to getInfo')
                return false
              }
            } catch {}
          }
        } catch (error) {
          console.log("Unexpected error during getInfo", error);
        }
      }
    },
  },
