  ...
  mutations: {
    ...
    addRecentUser(_, userEmail) {
      ...
      if (!isExist) {
        ...
        if (history.length > 5) {
          history.shift();  // Xóa email đầu tiên khi vượt quá giới hạn
        }
    ...
    async LoginAuthentication(context, payload) {
      ...
      try {
        ...
        if (status == "success") {
          ...
          if (isSave) {
            const encryptedEmail = CryptoJS.AES.encrypt(decoded.email, secret_key).toString();
            context.commit("addRecentUser", encryptedEmail);
          }
          return true;
        }
      } catch (error) {
