...
const localStorageEmail = computed(() => {
  console.log(localStorage.getItem("recentUsers"));
  
  return localStorage.getItem("recentUsers")
})

const emailUsers = ref(JSON.parse(localStorageEmail.value));
...
function removeAccount(emailRemove) {
  ...
  localStorage.setItem("recentUsers", JSON.stringify(userList));
}
onMounted(async () => {
