...
const onSubmit = handleSubmit(async (values) => {
  ...
  // phải kết hợp cả 2 localstorage và lưu ở vuex => hiệu quả: Theo dõi sự thay đổi
  const storageInfo = store.dispatch("auth/pushSignupInfo", signUpInfo);
  const isExits = await store.dispatch("auth/signUpAction", {
    ...signUpInfo,
    type: "check",
  });
  if (isExits == true && storageInfo) {
