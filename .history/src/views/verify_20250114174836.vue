...
// hanlde verify
const signupInfo = computed(() => {
  return store.getters["auth/getSignup"];
});
console.log(signupInfo);

// nếu đã xác thực thì trả về login
watch(
  signupInfo,
  (newValue) => {
...
async function handleSend(number) {
  ...
  const sendAction = await store.dispatch("auth/sendEmail", signupInfo);
  if (sendAction) {
    setTime(number);
  } else {
