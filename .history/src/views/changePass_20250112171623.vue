      ...
      <card title="Change your password">
        ...
        <template #footer>
          <div class="flex justify-end gap-4 items-center">
            ...
            >
            <baseButton class="bg-blue-500 text-white"> Confirm</baseButton>
          </div>
        </template>
      </card>
    </form>
...
const { defineField, handleSubmit } = useForm({
  ...
  validationSchema: yup.object({
    pass: yup
      ...
      .matches(/[a-z]/, "Password must contain at least one lowercase letter") // Kiểm tra có chữ thường
      .matches(/[A-Z]/, "Password must contain at least one uppercase letter"), // Kiểm tra có chữ hoa

    confirmPass: yup
      .string()
      .required("Confirm your password")
      .test("confirmPass-check", "Password do not match", (value) => {
        if (!value) return false;
        return values.pass !== values.confirmPass;
      }),
  }),
});
