        ...
        >
          <div class="big-box">
            <div class="left-box">
              ...
              <div class="recent_login mt-7 flex items-center justify-center gap-3">
                <div class="account_logged add_account border" v-for="user in recentUsers" :key="user.email">
                  <img :src="user.avatar" alt="user avt" class="object-contain">
                  <div class="bg-white flex-grow flex items-center justify-center ">
                    <p class="text-[#0866ff] text-lg">name</p>
                  </div>
                </div>
                <div
                  class="add_account"
    ...
    <Transition name="fade">
      ...
      >
        <loginForm class="!mx-auto !mt-28" :isShowRemember="true" @click.stop>
          <div class="border-b-2 p-4 my-2 flex items-center justify-between">
            <h1 class="text-xl text-center flex-grow">Log in to Facebook</h1>
            <i
              class="fa-solid fa-circle-xmark text-4xl hover:text-[#74C0FC] cursor-pointer"
              @click="isOpenForm = false"
            ></i>
          </div>
        </loginForm>
...
const emailUsers = JSON.parse(localStorage.getItem("recentUsers"));
async function usersInfo(emails) {
  const user = await store.dispatch("auth/getInfo", emails);
  return user;
}

onMounted(async () => {
  const recentUsers = await usersInfo(emailUsers);
