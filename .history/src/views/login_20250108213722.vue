            ...
            <div class="left-box">
              ...
              >
                ...
                >
                  ...
                  />
                  <div
                    class="bg-white border flex-grow flex items-center justify-center rounded-b-md"
                  >
    ...
    <Transition name="fade">
      ...
      >
        ...
        >
          <template #avatar>
            <i
                class="fa-solid absolute top-1 right-1 fa-circle-xmark text-4xl hover:text-[#74C0FC] cursor-pointer"
                @click="isOpenUserForm = false"
              ></i>
            <img
...
const emailUsers = ref(JSON.parse(localStorage.getItem("recentUsers")));
...
function removeAccount(emailRemove) {
  const userList = emailUsers.value.filter((email) => email != emailRemove);
  localStorage.setItem("recentUsers", userList);
}
onMounted(async () => {
  recentUsers.value = await usersInfo(emailUsers.value);
});
</script>
