<template>
  <!-- LIST COMMENT -->
  <section :class="`max-h-[${commentShow}px]`" class="relative">
    <div
      ...
      <div>
        <div class="inline-flex flex-col">
          ...
          >
            ...
            <span class="max-w-96">{{ comment.comment }}</span>
          </div>
          <div
        ...
        <!-- REPLY COMMENT -->
        <comment :commentsProp="comment.replies"></comment>
        <!-- END REPLY COMMENT -->
      </div>
   
    </div>
    <p v-if="formatCommentsTime(props.commentsProp).length " class="absolute font-bold hover:underline right-0 bottom-0 cursor-pointer shadow">Show more comments</p>
  </section>
...
const props = defineProps(["commentsProp"]);
...
// the height of comment section (px)
const commentShow = ref(200);
</script>
