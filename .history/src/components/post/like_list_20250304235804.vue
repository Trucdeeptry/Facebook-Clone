    ...
    >
      ...
      <div class="flex justify-start gap-3 mt-3 flex-col">
        ...
        >
          ...
          >
            <i
              :class="iconMap[activeEmotion]"
              :style="{ color: iconColorMap[activeEmotion] }"
            ></i>
          </div>
        </div>
      </div>
...
watch(activeEmotion, async (newVal) => {
  if (newVal !== "All") {
    ...
    const userId = reactsSort.filter((react) => react.count > 0).map(react => react.userId).flat();
    console.log(reactsSort);
    

    // users["All"] = await getUsers(userId);
  }
});
onMounted(async () => {});
</script>
