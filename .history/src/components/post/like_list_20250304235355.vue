    ...
    >
      ...
      <div class="flex justify-start gap-3 mt-3 flex-col">
        ...
        >
          ...
          >
          <div class="absolute flex justify-center items-center top-5 left-7 bg-white w-6 h-6 rounded-2xl">
            <i
...
watch(activeEmotion, async (newVal) => {
  if (newVal !== "All") {
    ...
    const userId = reactsSort.filter(react => react).userId
    user['All'] = await getUsers(userId)
  }
});
onMounted(async () => {});
</script>
