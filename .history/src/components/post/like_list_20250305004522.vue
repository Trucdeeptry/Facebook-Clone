  ...
  >
    ...
    >
      ...
      <div class="flex justify-start gap-3 mt-3 flex-col">
        <div
          class="flex relative items-center gap-3"
          v-for="user in users[activeEmotion]"
          :key="user._id"
        >
          <img
            :src="user.avatar"
            ...
            alt="User avatar"
          />
          <span class="font-bold dark:text-primary-txt">
            {{ user.info.firstName + " " + user.info.surName }}</span
          >
          <div
            class="absolute flex justify-center items-center top-5 left-7 bg-white w-6 h-6 rounded-2xl"
          >
            <i
              :class="
                activeEmotion === 'All'
                  ? iconMap[user.reactType]
                  : iconMap[activeEmotion]
              "
              :style="{
                color:
                  activeEmotion === 'All'
                    ? iconColorMap[user.reactType]
                    : iconColorMap[activeEmotion],
              }"
            ></i>
          </div>
        </div>
...
watch(
  ...
  async (newVal) => {
    ...
    if (users[newVal].length === 0) {
      if (newVal !== "All") {
        userId = reactsSort.filter((react) => react.type === newVal);
        users[newVal] = await getUsers(userId);
      } else {
        const filterReact = reactsSort.filter(react => react.count > 0);
        users[newVal] = await Promise.all(
          reactsSort.map(async (react) => {
            if (react.count > 0) {
