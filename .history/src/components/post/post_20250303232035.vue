    ...
    >
      ...
      <div class="px-4 py-2">
        <div class="flex items-center justify-between">
          ...
          >
            <div
              v-for="react in hanldeReactData(post.liked_by)"
              :key="react.type"
...
function hanldeReactData(reactList) {
  const likeReacts = reactList.filter((react) => react.type === "like");
  const heartReacts = reactList.filter((react) => react.type === "heart");
  const hahaReacts = reactList.filter((react) => react.type === "haha");
  const wowReacts = reactList.filter((react) => react.type === "wow");
  const sadReacts = reactList.filter((react) => react.type === "sad");
  const angryReacts = reactList.filter((react) => react.type === "angry");
  return [
    { type: "like", count: likeReacts.length },
    { type: "heart", count: heartReacts.length },
    { type: "haha", count: hahaReacts.length },
    { type: "wow", count: wowReacts.length },
    { type: "sad", count: sadReacts.length },
    { type: "angry", count: angryReacts.length },
  ];
}
onBeforeMount(async () => {
  try {
