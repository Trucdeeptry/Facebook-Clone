    ...
    >
      ...
      <div class="px-4 py-2">
        <div class="flex items-center justify-between">
          ...
          >
            <div
              v-for="react in post.liked_by"
              :key="react.react_type"
              class="ml-2 text-gray-500 dark:text-dark-txt hover:underline mr-2 cursor-pointer"
            >
              <i
                :class="iconMap[react.react_type]"
                :style="{ color: iconColorMap[react.react_type] }"
              ></i>
            </div>
          </div>
...
function hanldeData(likeCount) {
  data[0].count = likeCount;
}
onBeforeMount(async () => {
  try {
