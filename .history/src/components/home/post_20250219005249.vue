    ...
    >
      ...
      <div class="flex items-center justify-between px-4 py-2">
        <div class="flex space-x-2 items-center">
          <div class="relative">
            <a href="#">
              <img :src="post.author_info.avatar"  alt="Profile picture" class="object-cover w-8 h-8 rounded-full" />
              <span
          ...
          <div>
            <div class="font-semibold">
              <a href="#" class="hover:!underline">{{ post.author_info.info.firstName + " " + post.author_info.info.surName }}</a>
            </div>
      ...
      <!-- POST IMAGE -->
      <div class="py-2">
        <img src="/img/home/post.png" alt="Post image" />
      </div>
      <!-- END POST IMAGE -->
...
const formattedTime = computed(() => {
  const createdAt = dayjs(post.value.created_at);
  const now = dayjs();
  
  // Nếu hơn 7 ngày, hiển thị ngày tháng
  if (now.diff(createdAt, "day") > 7) {
    return createdAt.format("DD/MM/YYYY");
  } 
  // Nếu dưới 7 ngày, hiển thị thời gian tương đối (x phút trước, x giờ trước, ...)
  return createdAt.fromNow();
});
onBeforeMount(async () => {
  posts.value = await Promise.all(
    props.postsProp.map(async (posts) => {
      const author_info = await store.dispatch("auth/getInfo", posts.user_id);
      return {
        ...posts,
        author_info,
      };
