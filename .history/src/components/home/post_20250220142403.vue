      ...
      <div class="px-4 py-2">
        <div class="flex items-center justify-between">
          ...
          <div class="text-gray-500 dark:text-dark-txt flex items-center gap-3">
            <span>{{ post.postComments?.length }} <i class="fa-solid fa-comment"></i></span>
        ...
        >
          ...
          <div>
            <div>
              ...
              >
                <a href="#"  class="hover:underline font-semibold block">{{
...
function coutnComment(comments){  
  let count = comments.length
  for (const comment of comments){
    if(comment.replies.length > 0){
      count += coutnComment(comment.replies)
    }
  }
}
onBeforeMount(async () => {
