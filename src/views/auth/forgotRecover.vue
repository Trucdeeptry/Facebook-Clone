<template>
  <card title="We'll send you a code to your email address
      ">
    <template #body>
      <div class="flex justify-between">
        ...
        <div class="flex-grow flex flex-col items-center justify-center">
          <img :src="user.avatar" alt="avt" class="object-cover rounded-full w-20 h-20" />
          <div class="text-center w-full">
            <b class="block w-full">{{
              user.firstname + " " + user.surname
            }}</b>
            <p class="text-sm text-gray-500">TSocial user</p>
            <a href="#" @click="$router.push('/forgot-find')" class="text-blue-500 text-sm hover:underline">Not you?</a>
          </div>
        </div>
      </div>
    </template>
    ...
    <template #footer>
      <div class="flex items-center justify-between">
        <div class="flex flex-col gap-3">
          <resend second="60" :info="user.email" :run="triggerTime" @update:trigger="triggerTime = $event"
            @click="isSent = true"></resend>

          <b class="cursor-pointer text-blue-500 hover:underline text-md" @click="$router.push('/login')">
        ...
        <div class="flex gap-3 items-center">
          ...
          <baseButton @click="sendEmailRecover()" class="cursor-pointer" :class="[
            isSent
...
// trigger time này dùng để khi bấm vào continue thì ko được resend nữa
...
async function sendEmailRecover() {
  if (!isSent.value) {
    triggerTime.value = true;
    // khoá tạm nút continue trong lúc gửi email, tránh bấm nhiều lần
    isSent.value = true

    const isSendemailSuccess = await store.dispatch("auth/sendEmail", user.email);
