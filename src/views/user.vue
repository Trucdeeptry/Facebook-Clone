  <template>
    ...
    <div v-else>
      ...
      <div class="w-full min-h-[100vh] pb-20 bg-[#F1F2F5] dark:bg-dark-main">
          ...
          <div class="max-w-[1100px] pt-[56px] mx-auto pb-1">
            ...
            <div id="ProfileInfo" class="flex md:flex-row flex-col py-4 items-center justify-between px-4">
              <div class="flex md:flex-row flex-col gap-4 md:-mt-6 -mt-16 items-center">
                <div class="relative">
                  <img class="rounded-full w-[165px] h-[165px] border-white border-4 object-cover"
                ...
                <div class="md:mt-4 text-center md:text-left -mt-3">
                  ...
                  <div class="text-[17px] font-bold text-gray-600 dark:text-gray-300 mb-1.5 text-center md:text-left">
                    {{ userProfile.friends?.length }} friends
                  </div>

                  <div v-if="userProfile.friends?.length > 0" class="flex md:justify-start justify-center md:-ml-1">
                    <img v-for="friend in friendsInfo.slice(0, 8)" :key="friend.email"
                      class="rounded-full object-cover -ml-2 z-[1] w-[40px] h-[40px] border-white border-2"
                      :src="friend.avatar" />
                  </div>
                </div>
              </div>
              ...
              <div v-if="isOwner"
                ...
                <button class="flex justify-around cursor-pointer items-center w-full px-5 py-2 font-bold">
        ...
        <div class="flex-cols md:flex w-full max-w-[1100px] justify-between md:px-0 px-2 mx-auto">
          <div id="LeftSection" class="md:sticky h-fit -top-[100%] w-full md:w-5/12 mr-4">
            <user_card title="Intro">
              <p v-show="!isEditBio" class="text-center text-sm font-normal py-3">
                {{ userProfile.bio?.trim() }}
              </p>
              <div class="text-sm" v-if="isOwner">
                <div class="pb-5">
                  <button v-if="!userProfile.bio && !isEditBio" @click="isEditBio = true"
                  ...
                  <div v-else>
                    <div v-if="isEditBio">
                      <textarea v-model="editValue" maxlength="100"
            ...
            <user_card title="Friends">
              <div>
                ...
                <div v-if="friendsInfo.length > 0" class="grid grid-cols-3 cursor-pointer gap-3 py-2">
                  <router-link :to="`/user/${friend._id}`" v-for="friend in friendsInfo.slice(0, 9)"
                    class="w-32 h-32 mb-8 group" :key="friend.email">
                    <img :src="friend.avatar" alt="friends avt"
